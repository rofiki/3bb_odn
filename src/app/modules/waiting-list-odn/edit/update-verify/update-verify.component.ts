...
export class UpdateVerifyComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.service.findById(this.id).pipe().subscribe(item => {
      ...
      this.aform = this.fb.group({
        odn_verify_approve: this.fb.control('', [Validators.required]),
        odn_verify_users_id: this.fb.control('', [Validators.required]),
        odn_verify_date: this.fb.control(null, [Validators.required]),
      });

      this.localeService.use('th');
      if (this.itemRef.odn_verify_date) {
        this.aform.patchValue({
          odn_verify_date: new Date(this.itemRef.odn_verify_date),
        });
      }
  ...
  public async onSubmit() {
    ...
    params.odn_verify_date = this.datePipe.transform(params.odn_verify_date, 'yyyy-MM-dd hh:mm:ss');
    params.id = this.id;

    Swal.fire(swalOption.Confirm('Update Verify')).then((result) => {
      if (result.isConfirmed) {
        this.service.updateVerify(params).pipe(takeUntil(this.componentDestroyed$)).subscribe(r => {
          if (r.status) {
            Swal.fire({
              icon: "success",
              title: r.message,
