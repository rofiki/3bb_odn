...
@Component({
  selector: 'app-update-approve',
  templateUrl: './update-approve.component.html',
  styleUrls: ['./update-approve.component.scss']
})
export class UpdateApproveComponent {
  ...
  async ngOnInit() {
    ...
    this.service.findById(this.id).pipe().subscribe(item => {
      ...
      this.aform = this.fb.group({
        odn_approve_approve: this.fb.control('', [Validators.required]),
        odn_approve_date: this.fb.control('', [Validators.required]),
        odn_approve_users_id: this.fb.control(null, [Validators.required]),
        odn_approve_predict_date: this.fb.control(null, [Validators.required]),
      });

      this.localeService.use('th');
      if (this.itemRef.odn_approve_date) {
        this.aform.patchValue({
          odn_approve_date: new Date(this.itemRef.odn_approve_date),
          odn_approve_predict_date: new Date(this.itemRef.odn_approve_predict_date),
        });
      }
  ...
  public async onSubmit() {
    ...
    params.odn_approve_date = this.datePipe.transform(params.odn_approve_date, 'yyyy-MM-dd hh:mm:ss');
    params.odn_approve_predict_date = this.datePipe.transform(params.odn_approve_predict_date, 'yyyy-MM-dd hh:mm:ss');
    params.id = this.id;

    Swal.fire(swalOption.Confirm('Update Approve')).then((result) => {
      if (result.isConfirmed) {
        this.service.updateApprove(params).pipe(takeUntil(this.componentDestroyed$)).subscribe(r => {
          if (r.status) {
            Swal.fire({
              icon: "success",
              title: r.message,
